...
// Usage: node scripts/verify_app.js
...
async function verify() {
    ...
    try {
        ...
        const hasPurchase = ledgerEntries.some(e => e.voucher_type === 'PU' && e.credit == 1000000);
        const hasInvoice = ledgerEntries.some(e => e.voucher_type === 'IV' && e.debit == 20000);

        if (hasPurchase && hasInvoice) {
            console.log("\n✅ SUCCESS: All transactions verified in General Ledger!");
        } else {
            console.warn("\n⚠️  WARNING: Ledger verification partial or failed. Check transactions above.");
        }
